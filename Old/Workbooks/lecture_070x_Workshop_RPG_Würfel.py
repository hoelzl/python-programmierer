...
#
#
# In den folgenden Aufgaben sollen Sie derartige RPG-Würfel in Python implementieren.
# Um Ihre Implementierung testen zu können empfiehlt es sich sie in einem IDE zu realisieren.
...
#
# Schreiben Sie parametrische Pytest Tests, mit denen (indirekte) Instanzen von `Dice` getestet werden können.
#
# ## Klasse `ConstantDice`
#
...
#
# Was ist die Beziehung zwischen `FairDice` und der Kombination aus `SimpleDie` und `MultipleRollDice`? Wie unterscheiden sich die beiden Implementierungsstrategien in ihrer Testbarkeit.
#
